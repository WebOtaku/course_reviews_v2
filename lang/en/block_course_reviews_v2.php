<?php
// Общие настройки блока
$string['pluginname'] = 'Course Reviews (v. 2)';
$string['course_reviews_v2:addinstance'] = 'Add a new Course Reviews block (v. 2)';

$string['course_reviews_v2'] = 'Course Reviews (v. 2)';
$string['scos_course_reviews'] = 'SCOS Course Reviews (v. 2)';

$string['reviews_table'] = 'Reviews Table (v. 2)';
$string['scos_reviews_table'] = 'SCOS Reviews Table (v. 2)';

$string['emptytablemessage'] = 'The feedback_completed table does not contain any records associated with this course!';
$string['nopermissionmessage'] = 'You are not authorized to access this page!';

$string['savebtn'] = 'Save';
$string['acceptsavemessage'] = 'Are you sure you want to save?';

// Поля таблицы
$string['fbcid'] = 'ID';
$string['timemodified'] = 'Date';
$string['userid'] = 'User ID';
$string['firstname'] = 'First Name';
$string['lastname'] = 'Last Name';
$string['isvisible'] = 'Is Visible';